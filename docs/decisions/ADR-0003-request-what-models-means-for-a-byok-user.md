# ADR-0003 — What `/models` should mean for a BYOK product user

Status: **decided and built (cli-lead, 2026-09-14) — option (a), narrowed from the original proposal below; see
"Correction" section**
...
## Correction (2026-09-14, before item (2) was built) — `reachable` is not a live check

Item (2) above was wrong about what `panel.py`'s `reachable_in` actually does, and this was caught by reading
its body before wiring it in rather than after shipping it. For every BYOK provider (the `openai_compat`
interface — every provider relevant to this ADR), it makes **no network call at all**:

```python
if self.interface == "openai_compat" and self.credential:
    if os.environ.get(self.credential): return True, "key in environment"
    ...
    ...
    return False, f"{self.credential} is in none of {...} (bring your own key)"
```

`reachable` here means "is a key present" — the same fact `/api/providers`'s own `configured` field already
states, under a different name. Only the `cli` (`claude`/`codex` account status) and `local_gguf` (file
existence) interfaces do a real check, and neither applies to a BYOK user's own providers. The only place a
genuine provider-side validity check happens at all is `/settings`'s save-time `credentials.validate()` — a
one-shot HTTP probe that is not persisted or re-queryable afterward.

**Decision, corrected: option (a), narrowed.** Ship the providers view from item (1) only — configured/not,
per provider, with the real `/settings` CTA — and drop item (2)'s "Reachable" column entirely. A "Reachable"
label backed by `reachable_in` for a BYOK provider would be a second name for `configured` dressed up as a
live health check, which CHARTER §6 forbids as a dishonest surface. The real "is my key working right now"
signal is worth having but needs a genuine on-demand, read-only probe the engine does not have yet — filed as
[ADR-0004](./ADR-0004-request-two-open-model-visibility-gaps.md), not built here to avoid either shipping
nothing or faking it with a `validate()` call on every page load (rejected explicitly: a POST with a re-store
side effect, making outbound calls and spending the user's key quota on every visit).

## Related

- [ADR-0001 — This repository exists under Pravrudhi ADR-0049](./ADR-0001-this-repository-exists-under-pravrudhi-adr-0049.md)
- [ADR-0004 — Two open model-visibility gaps](./ADR-0004-request-two-open-model-visibility-gaps.md) — item (2)'s real on-demand health probe, and item (3)'s per-call attribution, both filed rather than built here
- `assistant/web/byok-provider-key-feedback` (`3673b63`) — the settings-page fix this ADR's item (1) builds on
- `assistant/web/product-models-providers-view` — this ADR's build
